...
export default function Navbar() {
    ...
    // আপনার সঠিক এডমিন পাথ এখানে সেট করা হয়েছে
        ...
        <header className="bg-bg-page/80 border-border sticky top-0 z-50 w-full border-b backdrop-blur-xl">
            <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4">
                ...
                <div className="flex items-center gap-8">
                    ...
                    {/* Desktop Navigation */}
                    <nav className="hidden items-center gap-1 md:flex">
                        {NAV_LINKS.filter((link) => link.name !== 'Feed' || isAuthenticated).map((link) => {
                                ...
                                <div key={link.name} className="group relative">
                                    <Link
                                        ...
                                        className={`relative flex items-center gap-1 rounded-lg px-3 py-2 text-sm font-semibold transition-all ${
                                            isActive ? 'text-text-primary bg-bg-subtle' : 'text-text-secondary hover:text-text-primary hover:bg-bg-subtle'
                                        }`}
                                    >
                                        {link.name}
                                        {link.subLinks && <ChevronDown size={12} className="group-hover:rotate-180 transition-transform" />}
                                        {isActive && <span className="bg-accent absolute right-3 bottom-1 left-3 h-0.5 rounded-full" />}
                                    </Link>

                                    {link.subLinks && (
                                        <div className="bg-bg-page border-border absolute top-full left-0 z-50 mt-1 min-w-[160px] translate-y-2 scale-95 overflow-hidden rounded-xl border p-1 opacity-0 shadow-2xl transition-all group-hover:translate-y-0 group-hover:scale-100 group-hover:opacity-100">
                                            {link.subLinks.map((sub) => (
                                                <Link key={sub.name} href={sub.href} className="text-text-secondary hover:text-text-primary hover:bg-bg-subtle flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold transition-all">
                                                    {sub.name}
                                                </Link>
                                            ))}
                                        </div>
                        ...
                        {isAuthenticated && (
                            <Link
                                href={adminPath}
                                className={`flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-bold transition-all border ml-2 ${
                                    pathname.startsWith('/admin') 
                                    ? 'bg-accent text-white border-accent shadow-[0_0_15px_rgba(var(--accent-rgb),0.3)]' 
                                    : 'text-accent border-accent/20 hover:bg-accent/10'
                                }`}
                            >
                ...
                <div className="hidden items-center gap-3 md:flex">
                    ...
                    {isAuthenticated && user ? (
                        ...
                        <>
                            <Link href="/login" className="text-text-secondary hover:text-text-primary px-3 py-2 text-sm font-semibold transition-colors">
                                Sign in
                            </Link>
                            <Link href="/signup">
                ...
                <div className="flex items-center gap-2 md:hidden">
                    <ThemeToggle />
                    {isAuthenticated && (
                        <Link href={adminPath} className={`mr-1 p-2 rounded-lg border transition-all ${pathname.startsWith('/admin') ? 'bg-accent text-white border-accent' : 'bg-accent/10 text-accent border-accent/20'}`}>
