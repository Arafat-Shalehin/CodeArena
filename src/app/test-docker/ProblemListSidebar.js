...
const DIFFICULTY_LABELS = {
    ...
    medium: { text: 'Medium', color: 'text-warning' },
    hard: { text: 'Hard', color: 'text-error' },
}
...
export default function ProblemListSidebar({
    ...
    const filteredProblems = problems
        ...
        .sort((a, b) => {
            ...
            const weightA = DIFFICULTY_WEIGHTS[a.difficulty?.toLowerCase()] || 2
            const weightB = DIFFICULTY_WEIGHTS[b.difficulty?.toLowerCase()] || 2

            if (weightA !== weightB) {
                return weightA - weightB
            }

            // Secondary Sort: Title (A-Z or Z-A)
                        ...
                        <div className="border-border flex flex-shrink-0 flex-col border-b">
                            ...
                            <div className="flex items-center justify-between px-5 py-4">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <button
                                        onClick={onClose}
                                        className="text-text-muted hover:bg-bg-muted hover:text-text-primary rounded-md p-1 transition-colors"
                                    >
                                        <X size={18} />
                                    </button>
                                </div>
                            ...
                            <div className="flex items-center gap-2 px-5 pb-3">
                                {/* Search */}
                                <div className="bg-bg-muted flex flex-1 items-center gap-2 rounded-lg px-3 py-2">
                                ...
                                <div className="relative" ref={filterRef}>
                                    <button
                                        ...
                                        className={`hover:bg-bg-muted relative rounded-lg p-2 transition-colors ${filterDifficulty ? 'text-warning' : 'text-text-muted hover:text-text-primary'}`}
                                        title="Filter by difficulty"
                                    >
                                        <Filter size={16} />
                                        {filterDifficulty && (
                                            <span className="bg-error absolute -top-0.5 -right-0.5 h-2 w-2 rounded-full" />
                                        )}
                                    </button>
                                    {showFilterDropdown && (
                                        <div className="bg-bg-subtle border-border absolute top-full right-0 z-10 mt-1 w-32 rounded-lg border py-1 shadow-xl">
                                            <button
                                                onClick={() => {
                                                    setFilterDifficulty(null)
                                                    setShowFilterDropdown(false)
                                                }}
                                                className={`text-text-muted hover:bg-bg-muted w-full px-3 py-1.5 text-left text-xs ${!filterDifficulty ? 'bg-bg-muted text-text-primary' : ''}`}
                                            >
                                                All
                                            </button>
                                            {Object.entries(DIFFICULTY_LABELS).map(
                                                ([key, { text, color }]) => (
                                                    <button
                                                        ...
                                                        className={`hover:bg-bg-muted w-full px-3 py-1.5 text-left text-xs ${filterDifficulty === key ? 'bg-bg-muted' : ''} ${color}`}
                                                    >
                                                        {text}
                                                    </button>
                                                )
