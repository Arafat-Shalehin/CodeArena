        ...
        <div className="w-full max-w-md mx-auto space-y-4">
            ...
            <form className="space-y-3" onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="space-y-1">
                    <div className="flex items-center gap-1.5">
                        ...
                        <div className="relative group">
                            <AlertCircle className="h-3.5 w-3.5 text-text-muted hover:text-accent cursor-help" />
                            <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-48 p-2 bg-text-primary text-bg-page text-xs rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 text-center pointer-events-none">
                ...
                <div className="space-y-1">
                    <div className="flex items-center gap-1.5">
                        ...
                        <div className="relative group">
                            <div className={cn("cursor-help transition-colors", strength === 4 ? "text-success" : "text-text-muted hover:text-warning")}>
                                <AlertCircle className="h-3.5 w-3.5" />
                            </div>
                            <div className="absolute left-0 bottom-full mb-2 w-56 p-3 bg-bg-page border border-border rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 pointer-events-none">
                                <p className="text-xs font-semibold mb-2 text-text-primary">Password Requirements</p>
                                <div className="space-y-1.5">
                                    <div className={cn("flex items-center gap-2 text-xs", password.length >= 8 ? "text-success" : "text-text-muted")}>
                                        <CheckCircle className="h-3 w-3 shrink-0" /> 8+ characters
                                    </div>
                                    <div className={cn("flex items-center gap-2 text-xs", /[A-Z]/.test(password) ? "text-success" : "text-text-muted")}>
                                        <CheckCircle className="h-3 w-3 shrink-0" /> One uppercase
                                    </div>
                                    <div className={cn("flex items-center gap-2 text-xs", /[0-9]/.test(password) ? "text-success" : "text-text-muted")}>
                                        <CheckCircle className="h-3 w-3 shrink-0" /> One number
                                    </div>
                                    <div className={cn("flex items-center gap-2 text-xs", /[^A-Za-z0-9]/.test(password) ? "text-success" : "text-text-muted")}>
                                        <CheckCircle className="h-3 w-3 shrink-0" /> Special char
                                    </div>
                                </div>
                                <div className="absolute top-full left-1 border-4 border-transparent border-t-bg-page drop-shadow-sm"></div>
                            </div>
                        </div>
                    </div>
                    <div className="relative">
                        ...
                        <div className="pt-1">
                            <div className="flex items-center justify-between mb-0.5">
                                <span className="text-[10px] font-medium text-text-muted">
                                    Strength: <span className={cn("font-bold", getStrengthTextColor())}>{getStrengthText()}</span>
                                </span>
                            </div>
                            <div className="flex h-1 w-full gap-1">
                                {[0, 1, 2, 3].map((i) => (
                                    <div key={i} className={cn("h-full flex-1 rounded-full", getStrengthColor(i))} />
                                ))}
                            </div>
                        </div>
                    )}
            ...
            <div className="relative">
                ...
                <div className="relative flex justify-center text-[10px] uppercase">
                    <span className="bg-bg-page px-2 text-text-muted">Or</span>
                </div>
            </div>
            ...
            <p className="text-center text-sm text-text-secondary">
                ...
                <Link
                    href="/login"
                    className="font-bold text-accent hover:text-accent-hover hover:underline underline-offset-4 transition-colors"
                >
                    Sign In
                </Link>
            </p>
